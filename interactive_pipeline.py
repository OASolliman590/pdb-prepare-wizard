...
def run_single_pdb_analysis(pipeline, pdb_id, excel_workbook=None):
    ...
    try:
        ...
        if selected_hetatm and selected_hetatm in to_remove_list:
            print(f"⚠️  WARNING: You selected {selected_hetatm} as your ligand but it's also in the removal list!")
            keep_ligand = get_user_choice("Remove it from cleaning list?", ['y', 'n'], 'y')
            if keep_ligand == 'y':
        ...
        # Step 5: Extract active site coordinates (if ligand selected)
        ...
        if selected_hetatm:
            try:
                ...
                # Step 6: Analyze pocket properties
